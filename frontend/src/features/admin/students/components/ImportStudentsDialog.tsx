...
function downloadTemplate() {
  const ws = XLSX.utils.aoa_to_sheet([
    ...
    ["Nguyễn Văn A", "0901234567", "0911234567", "vana@gmail.com", "Toán 10A, IELTS-01"],
    ["Trần Thị B", "0912345678", "", "thib@gmail.com", "Hóa 11-A"],
  ]);
    ...
    reader.onload = (e) => {
      try {
        ...
        const parsed: ParsedRow[] = raw.map((r, idx) => {
          ...
          const parentPhone = String(r["SĐT Phụ huynh"] ?? "").trim();
          ...
          else if (!/^0\d{9}$/.test(phone)) errors.push("SĐT không hợp lệ");
  ...
  const handleImport = async () => {
    ...
    for (let i = 0; i < selected.length; i++) {
      ...
      try {
        ...
        // skip failed rows silently, counted at the end
      }
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 bg-secondary/50 rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">File có sẵn tiêu đề cột và 2 dòng ví dụ minh họa</p>
                </div>
